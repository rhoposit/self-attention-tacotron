...
class DualSourceSelfAttentionTacotronModel(tf.estimator.Estimator):
    ...
    def __init__(self, params, model_dir=None, config=None, warm_start_from=None):
        def model_fn(features, labels, mode, params):
            ...
            code_output, stop_token, decoder_state = decoder((encoder_lstm_output, encoder_self_attention_output),
...
#                code_output = tf.Print(code_output, [code_output, tf.shape(code_output)], "\ncode output")
#                out2 = tf.Print(labels.codes, [labels.codes, tf.shape(labels.codes)], "\nlabels.codes")
#                out3 = tf.Print(labels.code_loss_mask, [labels.code_loss_mask, tf.shape(labels.code_loss_mask)], "\nlabels.code_mask")
...
#                code_loss = codes_loss(code_output, out2, out3,params.code_loss_type)
            ...
            if is_validation:
                ...
                attention1_fn, attention2_fn = dual_source_attention_factory(params)
                code_output_with_teacher, stop_token_with_teacher, decoder_state_with_teacher = decoder(
